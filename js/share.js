...
const Share = {
    platforms: {
        ...
        email: {
            ...
            getUrl: (url, text) => `mailto:?subject=${encodeURIComponent(text)}&body=${encodeURIComponent('Check out this biography: ' + url)}`
        }
    },

    share(platform, url = window.location.href, text = document.title) {
    ...
    async nativeShare(data = {}) {
        const shareData = {
            ...
            text: data.text || 'Check out this biography on Womencypedia',
        ...
        if (navigator.share) {
            try {
                ...
                this.trackShare('native', shareData.url);
                return true;
            } catch (error) {
    ...
    trackShare(platform, url) {
        try {
            const shares = JSON.parse(localStorage.getItem('womencypedia_shares') || '[]');
            shares.push({ platform, url, timestamp: new Date().toISOString() });
            localStorage.setItem('womencypedia_shares', JSON.stringify(shares.slice(-50)));
        } catch (e) { }
    },

    renderShareButtons(container, options = {}) {
        if (!container) return;
        const { url = window.location.href, text = document.title, size = 'md' } = options;
        const platforms = options.platforms || ['twitter', 'facebook', 'linkedin', 'whatsapp', 'email'];

        container.innerHTML = `
            <div class="flex items-center gap-2">
                ${navigator.share ? `
                    <button onclick="Share.nativeShare({url: '${url}', text: '${text}'})" class="p-2 bg-gradient-to-r from-pink-500 to-purple-600 text-white rounded-full hover:opacity-90">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8.684 13.342C8.886 12.938 9 12.482 9 12c0-.482-.114-.938-.316-1.342m0 2.684a3 3 0 110-2.684m0 2.684l6.632 3.316m-6.632-6l6.632-3.316m0 0a3 3 0 105.367-2.684 3 3 0 00-5.367 2.684zm0 9.316a3 3 0 105.368 2.684 3 3 0 00-5.368-2.684z"/></svg>
                    </button>
                ` : ''}
                ${platforms.map(p => `
                    <button onclick="Share.share('${p}', '${url}', '${text}')" class="p-2 bg-white/10 text-gray-400 rounded-full hover:bg-white/20 hover:text-white transition-colors" title="Share on ${this.platforms[p].name}">
                        ${this.platforms[p].icon}
                    </button>
                `).join('')}
                <button onclick="Share.copyLink('${url}')" class="p-2 bg-white/10 text-gray-400 rounded-full hover:bg-white/20 hover:text-white transition-colors" title="Copy link">
