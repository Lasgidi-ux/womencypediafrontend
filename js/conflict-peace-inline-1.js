const COLLECTION_CATEGORY = 'Conflict & Peace';
        ...
        async function loadCollection() {
            ...
            try {
                ...
                if (typeof DATA !== 'undefined' && DATA.entries) {
                    const filtered = DATA.entries.filter(e => e.category === COLLECTION_CATEGORY || (e.tags && e.tags.some(t => t.toLowerCase().includes('warrior') || t.toLowerCase().includes('peace'))));
                    loadingState.classList.add('hidden');
        ...
        function renderEntries(entries) {
            document.getElementById('entries-grid').innerHTML = entries.map(entry => `
                <a href="../biography.html?id=${entry.id || entry.entry_id}" class="bg-white rounded-2xl border border-border-light overflow-hidden hover:shadow-lg transition-all group">
                    <div class="h-48 bg-gradient-to-br from-accent-gold/20 to-red-100/30 flex items-center justify-center">
                        ${entry.image_url || entry.image ? `<img src="${entry.image_url || entry.image}" alt="${entry.full_name || entry.name}" class="w-full h-full object-cover">` : `<span class="material-symbols-outlined text-accent-gold/30 text-6xl">person</span>`}
                    </div>
                    <div class="p-6">
                        <h3 class="font-serif text-lg font-bold text-text-main group-hover:text-accent-gold transition-colors mb-2">${entry.full_name || entry.name}</h3>
                        ...
                        <span class="px-2 py-1 bg-accent-gold/10 text-accent-gold text-xs rounded-full">${entry.region || 'Global'}</span>
                    </div>
                </a>
            `).join('');
        }
